...
import { Shield, Clock, Terminal, User, RefreshCw, AlertTriangle, Search, Activity, Archive, Trash2, RotateCcw } from "lucide-react";
...
export default function AdminPanelView({ currentUser, onResetDB, showToast }: AdminPanelViewProps) {
  ...
  const fetchArchive = async () => {
    ...
    try {
      const res = await fetch("/api/customers?includeDeleted=true");
      if (res.ok) {
        const data = await res.json();
        const deleted = data.filter((c: any) => c.deletedAt).map((c: any) => ({
          id: c.id,
          name: c.name,
  ...
  const handleRestoreCustomer = async (id: string) => {
    try {
      ...
      if (res.ok) {
        showToast("Müştəri uğurla arxivdən çıxarıldı", "success");
        fetchArchive();
        fetchLogs();
      } else {
        const data = await res.json();
  ...
  const handleForceDeleteCustomer = async (id: string) => {
    if (!window.confirm("Bu müştərini tamamilə silmək istədiyinizə əminsiniz? Qaimələr və ödənişlər də silinəcək.")) return;
    try {
      const savedUser = localStorage.getItem("erp_user");
      const userObj = savedUser ? JSON.parse(savedUser) : null;
      const res = await fetch(`/api/customers/${id}/force`, {
        method: "DELETE",
        headers: {
          "x-user-role": userObj?.role || "",
          "x-user-username": userObj?.username || ""
      ...
      if (res.ok) {
        showToast("Müştəri tamamilə silindi", "success");
        fetchArchive();
        fetchLogs();
      } else {
        const data = await res.json();
    ...
    <div className="flex-1 flex flex-col h-full bg-slate-50 overflow-hidden">
      ...
      <div className="flex-1 overflow-hidden p-6 flex flex-col space-y-4">
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-4 gap-4 overflow-hidden">
          ...
          <div className="lg:col-span-1 bg-white rounded-xl border border-slate-200 shadow-xs flex flex-col overflow-hidden">
            <div className="p-4 border-b border-slate-200 bg-slate-50/50 flex items-center justify-between">
              <span className="font-bold text-xs text-slate-700 uppercase tracking-wider flex items-center">
                <Archive className="w-3.5 h-3.5 mr-1.5 text-slate-500" />
                Müştəri Arxiv
              </span>
              <span className="text-[10px] bg-slate-200 text-slate-600 font-bold px-2 py-0.5 rounded-full border border-slate-300">{deletedCustomers.length}</span>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {loadingArchive ? (
                <div className="text-xs text-center text-slate-400">Yüklənir...</div>
              ) : deletedCustomers.length === 0 ? (
                <div className="text-xs text-center text-slate-400">Arxiv boşdur</div>
              ) : (
                deletedCustomers.map(c => {
                  const daysLeft = 30 - Math.floor((new Date().getTime() - new Date(c.deletedAt).getTime()) / (1000 * 3600 * 24));
                  return (
                    <div key={c.id} className="p-3 rounded-lg border border-slate-200 bg-white space-y-2 relative overflow-hidden group">
                      <div className="flex justify-between items-start">
                        <div className="font-bold text-xs text-slate-800">{c.name}</div>
                        <div className="text-[9px] font-bold px-1.5 py-0.5 bg-rose-50 text-rose-600 rounded">{daysLeft} gün qalıb</div>
                      </div>
                      <div className="flex justify-between mt-2">
                        <button onClick={() => handleRestoreCustomer(c.id)} className="text-[10px] bg-emerald-50 text-emerald-600 hover:bg-emerald-100 px-2 py-1 rounded font-bold transition flex items-center cursor-pointer">
                          <RotateCcw className="w-3 h-3 mr-1" />
                          Geri qaytar
                        </button>
                        <button onClick={() => handleForceDeleteCustomer(c.id)} className="text-[10px] bg-rose-50 text-rose-600 hover:bg-rose-100 px-2 py-1 rounded font-bold transition flex items-center cursor-pointer">
